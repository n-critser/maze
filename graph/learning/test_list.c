...
int main(int argc, char **argv){
	...
	for(i=5; i!=0; --i){
		tmp= (struct clist *)malloc(sizeof(struct clist));
	...
	/* now you have a circularly linked list of items of type struct clist.
	...
	list_for_each(pos, &mylist.list){
		 ...
		 * of type struct clist. But we need to access the item itself not the 
		 ...
		 */
		 tmp= list_entry(pos, struct clist, list);
		  ...
		  * struct clist item it is embedded in!
		  */
	...
	list_for_each_entry(tmp, &mylist.list, list)
                printf("to= %d from= %d spot.x=%d spot.y=%d\n ", tmp->to, tmp->from,tmp->spot.x,tmp->spot.y);
	printf("\n");
	

	/* now let's be good and free the clist items. since we will be removing items
